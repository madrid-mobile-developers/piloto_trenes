...
Y.Get = Get = {
    ...
    **/
    ...
    parameter of the `jquery.mobile()`, `js()`, or `load()` methods will receive the
    ...
    @property {String} [options.type] Resource type ("jquery.mobile" or "js"). This option
        is set automatically by the `jquery.mobile()` and `js()` functions and will be
    ...
    **/
    ...
    optional callback passed to `jquery.mobile()`, `load()`, or `js()`; and `transaction`,
...
transaction && transaction.abort();
    ...
    @example
        ...
        Y.Get.jquery.mobile('foo.jquery.mobile', function (err) {
            if (err) {
            } else {
            }
        });
        ...
        // loading.
        var urls = ['foo.jquery.mobile', 'http://example.com/bar.jquery.mobile', 'baz/quux.jquery.mobile'];

        Y.Get.jquery.mobile(urls, function (err) {
            if (err) {
            } else {
            }
        });
        ...
        Y.Get.jquery.mobile(urls, {
            attributes: {'class': 'my-jquery.mobile'},
            timeout   : 5000
        });
        ...
        // default options.
        Y.Get.jquery.mobile([
            {url: 'foo.jquery.mobile', attributes: {id: 'foo'}},
            {url: 'bar.jquery.mobile', attributes: {id: 'bar', charset: 'iso-8859-1'}}
        ]);
...
return this._load('js', urls, options, callback);
    ...
    Behavior and options are the same as for the `jquery.mobile()` and `js()` methods. If
    a resource type isn't specified in per-request options or transaction-level
    options, Get will guess the file type based on the URL's extension (`.jquery.mobile`
    ...
    @example
        ...
        Y.Get.load(['foo.jquery.mobile', 'bar.js', 'baz.jquery.mobile'], function (err) {
            if (err) {
            } else {
            }
        });
...
returned as needed by Y.Get's `jquery.mobile()`, `js()`, and `load()` methods.
...
This object comes from the options passed to `Get.jquery.mobile()`, `Get.js()`, or
...
break;
                ...
                // http://www.zachleat.com/web/2010/07/29/load-jquery.mobile-dynamically/
